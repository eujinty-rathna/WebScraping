...
class RedtapeshoesSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        def get_amount(price_string):
            return  Price.fromstring(price_string).amount_float
        ...
        price = get_amount(response.xpath('//h2/text()').re_first(r'.*\d+'))
        symbol = get_symbol(response.xpath('//h2/text()').re_first(r'.*\d+'))
        sizes = response.xpath('//div[@class="radio span-option-div "]/label/span/text()').getall()
        mrp = get_amount(parse_with_xpath('//span[@style="text-decoration: line-through;"]/text()'))
